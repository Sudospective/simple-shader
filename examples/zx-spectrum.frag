...
vec3 fmap(vec4 c) {
  return c.a>=0.5 ? c.rgb*vec3(CFULL,CFULL,CFULL) : c.rgb*vec3(CHALF,CHALF,CHALF);
}


void zxspectrum_clash() {
  vec2 pv = floor(gl_FragCoord.xy/LOWREZ);
  ...
  for(int py=1;py<8;py++) {
    for(int px=0;px<8;px++) {
      vec4 cs=bmap( (texture(sampler0,(bv+vec2(px,py))/sv).rgb) );
      bright+=cs.a;
  ...
  vec3 cs=texture(sampler0,pv/sv).rgb;
...
void zxspectrum_colors() {
  ...
  vec4 cs=smap(texture(sampler0,pv/sv).rgb);
