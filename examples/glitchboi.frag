...
void main() {
  ...
  //id.x /= floor(texture(sampler0, vec2(id / 8.0)).r * 8.0);
  ...
  float colR = texture(sampler0, vec2(uv.x + shift, uv.y)).r * (1. - shift) ;
  float colG = texture(sampler0, vec2(uv.x - shift, uv.y)).g * (1. - shift) + rand(id) * shift;
  float colB = texture(sampler0, vec2(uv.x - shift, uv.y)).b * (1. - shift);
